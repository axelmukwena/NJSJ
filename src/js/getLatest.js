...
function fetchData() {
    fetch('/volumes/latest').then((response) => {
        response.json().then((data) => {
            ...
            document.querySelector('#latestVolLink').insertAdjacentHTML('beforeend', `<a href="/volumes/editorial/${data._id}" class="btn btn-primary" id="latestLink">Download Volume 1</a> <a href="/volume/${data._id}" class="btn btn-primary" id="latestLink">View Articles</a>`)
            ...
            fetch('/articles/volume/' + data._id + '?sortBy=createdAt:asc&feature=true').then((response) => {
                response.json().then((data) => {
